...
static VALUE task_context_do_port(VALUE self, VALUE name)
{
    ...
    RTT::TypeInfo* ti = get_type_info(static_cast<char const*>(type_name));
    if (!ti)
        rb_raise(rb_eArgError, "%s is not registered in the RTT type system, has the typekit been generated by orogen ?", static_cast<char const*>(type_name));
...
static VALUE do_input_writer_write(VALUE port_access, VALUE type_name, VALUE rb_typelib_value)
{
    ...
    else
    {
        ...
        transport->writePort(local_port, handle);
        transport->deleteHandle(handle);
    }
