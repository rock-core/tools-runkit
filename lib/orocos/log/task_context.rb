    ...
    module Log
        ...
        class OutputReader
            ...
            def initialize(port, policy = default_policy)
                @policy = default_policy unless policy
                @port = port
                @buffer = []
            ...
            def update(raw_data)
                if @policy_type == :buffer
                    @buffer << raw_data if @buffer.size != @buffer_size
        ...
        class OutputPort < InterfaceObject
            ...
            class Connection #:nodoc:
                ...
                def initialize(log_port, port, policy = {})
                    @log_port = log_port
                    @port = port
                    policy ||= OutputPort.default_policy
                    @filter, policy = Kernel.filter_options(policy, [:filter])
                    @filter = @filter[:filter]
            ...
            def pretty_print(pp)
                ...
                pp.nest(2) do
                    ...
                    pp.breakable
                    pp.text "filtered = #{!@filter.nil?}"
                    @connections.each do |connection|
                        pp.breakable
                        pp.text "connected to #{connection.port.task.name}.#{connection.port.name} (filtered = #{!connection.filter.nil?})" if connection.is_a?(OutputPort::Connection)
                        pp.text "connected to code block" if connection.is_a?(OutputPort::CodeBlockConnection)
                    end
                end
            end
            ...
            def raw_read
                raise "port #{full_name} is not replayed. Set tracked to true or use a port reader!" unless used?
            ...
            def connect_to(port = nil, policy = OutputPort.default_policy, &block)
                port = if port.respond_to? :find_input_port
                           ...
                           unless (result = port.find_input_port(type, nil))
                ...
                Orocos::Log.warn "connect_to to a code block { |data| ... } is deprecated. Use #on_data instead." if block && !port
                ...
                if !port
                    ...
                    raise "Cannot connect to #{port.class}" unless port.instance_of?(Orocos::InputPort)
        ...
        class Property < InterfaceObject
            ...
            def orocos_type_name
                if metadata&.key?(:rock_orocos_type_name)
        ...
        class TaskContext < Orocos::TaskContextBase
            ...
            def initialize(log_replay, task_name, file_path = nil, file_path_config = nil)
                ...
                @invalid_ports = {} # ports that could not be loaded
                @rtt_state = :RUNNING
                @port_reachable_blocks = []
            ...
            def add_property(stream, _backward = nil)
                ...
                if @properties.key?(log_property.name)
            ...
            def track(value, filter = {})
                options, filter = Kernel.filter_options(filter, %i[ports types limit])
            ...
            # Clears all reader buffers
            def clear_reader_buffers
                @ports.each_value(&:clear_reader_buffers)
            end
            ...
            def method_missing(m, *args, &block) #:nodoc:
                ...
                return property(m) if has_property?(m)
