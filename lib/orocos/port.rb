    ...
    class Port
        ...
        def handle_mq_transport(input_name, policy) # :nodoc:
            if policy[:transport] == TRANSPORT_MQ
                raise InvalidMQTransportSetup, "cannot select the MQueue transport as it is not built into the RTT" unless Orocos::MQueue.available?
            ...
            if Orocos::MQueue.auto_sizes? && message_size == 0
                ...
                unless size
                    ...
                    Orocos.warn "the MQ transport could be selected, but the marshalling size of samples from the output port #{full_name}, of type #{type.name}, is unknown, falling back to auto-transport" if Orocos::MQueue.warn?
                    return policy.dup
                end
            ...
            if Orocos::MQueue.validate_sizes?
                ...
                unless valid
                    ...
                    Orocos.warn "the MQ transport could be selected, but the marshalling size of samples (#{policy[:data_size]}) is invalid, falling back to auto-transport" if Orocos::MQueue.warn?
                    return policy.dup
                end
            end
