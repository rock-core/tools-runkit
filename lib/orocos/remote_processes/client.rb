    ...
    module RemoteProcesses
        ...
        class Client
            ...
            def pid(timeout: @response_timeout)
                ...
                raise "timeout while reading process server at '#{host}:#{port}'" unless select([socket], [], [], timeout)
            ...
            def wait_for_answer(timeout: @response_timeout)
                loop do
                    raise TimeoutError, "reached timeout of #{timeout}s in #wait_for_answer" unless select([socket], [], [], timeout)
            ...
            def start(process_name, deployment, name_mappings = {}, options = {})
                raise ArgumentError, "this client already started a process called #{process_name}" if processes[process_name]

                if deployment.respond_to?(:to_str)
                    deployment_model = loader.root_loader.deployment_model_from_name(deployment)
                    raise OroGen::DeploymentModelNotFound, "deployment #{deployment} exists locally but not on the remote process server #{self}" unless loader.has_deployment?(deployment)
            ...
            def wait_termination(timeout = nil)
                if @death_queue.empty?
                    ...
                    while reader
                        raise ComError, "communication to process server closed" if socket.eof? # remote closed, probably a crash
