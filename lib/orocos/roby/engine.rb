	    ...
	    def enable_updates; @enabled = true end
            ...
            def resolve_name(name, instance_name = nil)
                ...
                if device = robot.devices[model_name]
                    ...
                    raise NameResolutionError.new(name, robot.devices.keys + defines.keys), "#{name} does not refer to a known task or device (known tasks: #{tasks.keys.sort.join(", ")}; known devices: #{robot.devices.keys.sort.join(", ")})"
                end
