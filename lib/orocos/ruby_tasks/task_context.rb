...
module Orocos
    module RubyTasks
        ...
        class TaskContext < Orocos::TaskContext
            ...
            #   what you are doing
            def create_input_port(name, orocos_type_name, options = {})
            ...
            #   represent the port on the Ruby side. Do not change unless you know
            #   what you are doing
            def create_output_port(name, orocos_type_name, options = {})
            ...
            def setup_from_orogen_model(orogen_model)
                ...
                orogen_model.each_property do |p|
                    if has_property?(p.name)
                        raise IncompatibleInterface, "cannot adapt the interface of #{self} to match the model in #{orogen_model}: #{self} already has a property called #{p.name}, but with a different type" if property(p.name).orocos_type_name != p.orocos_type_name
