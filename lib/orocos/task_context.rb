...
module Orocos
    ...
    class TaskContext
	    ...
	    begin
		if !model
		    raise NoModel, "#{self} does not provides its model name through a getModelName operation. It is most likely not generated by oroGen. orocos.rb currently requires this"
		end
	    rescue Orocos::NotFound
		raise NoModel, "#{self} does provide its model name (#{getModelName}), but it is not available on this system. orocos.rb currently requires this"
	    end
        end
        ...
        # See also #info
        def model
            if !super && has_operation?("getModelName")
                model_name = self.getModelName
                self.model = Orocos.task_model_from_name(model_name)
            end
            @model
        end
    end
