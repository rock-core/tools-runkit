...
module Runkit
    ...
    # Setup logging on all output ports of the processes started with Runkit.run
    #
    # This method is designed to be called within an Runkit.run block
    #
    ...
    #   Runkit.log_all_ports(exclude_ports: /^io_/)
    # @example log all ports whose type name does not contain 'debug'
    #   Runkit.log_all_ports(exclude_types: /debug/)
    #
    ...
    # Runkit.log_all_configuration. It will automatically be created by
    # Oroocs.configuration_log if Runkit.log_all_configuration is called without
    # an argument
    attr_writer :configuration_log

    # If Runkit.configuration_log is not set, it creates a new configuration log
    # file that will be used by default by Runkit.log_all_configuration. The log
    # file is named as Runkit.configuration_log_name ('task_configuration' by
    # default). This log file becomes the new default log file for all following
    # calls to Runkit.log_all_configuration
    def self.configuration_log
        ...
        @configuration_log ||= Pocolog::Logfiles.create(File.expand_path(Runkit.configuration_log_name, Runkit.default_working_directory))
    end
    ...
    def self.log_all_process_ports(process, tasks: nil, exclude_ports: nil, exclude_types: nil, **logger_options)
        ...
        process.task_names.each do |task_name|
            ...
            task.each_output_port do |port|
                ...
                Runkit.info "logging % 50s of type %s" % ["#{task.name}:#{port.name}", port.type.name]
