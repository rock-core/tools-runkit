...
module Runkit
    ...
    module MQueue
            ...
            if @available.nil?
                @available =
                    if !defined?(RTT_TRANSPORT_MQ_ID)
                        ...
                        raise InternalError, "hardcoded value of TRANSPORT_MQ differs from the transport ID from RTT (#{TRANSPORT_MQ} != #{RTT_TRANSPORT_MQ_ID}. Set the value at the top of #{File.expand_path(__FILE__)} to #{RTT_TRANSPORT_MQ_ID} and report to the runkit.rb developers"
                    else
                        true
                    end
            else
        ...
        class << self
            ...
            # :method:auto?
            # :call-seq:
            #   Runkit::MQueue.auto? => true or false
            #   Runkit::MQueue.auto = new_value
            #
            # If true, runkit.rb will try to use the MQ transport when
            # * the max sample size on the output port is known. If the port type
            #   contains variable-sized containers, it means that
            #   {Runkit::OutputPort#max_size} and/or OroGen::Spec::OutputPort#max_size
            ...
            #   is possible only if runkit.rb is used to start the processes)
            #
            # It is false by default. Additionally, the Runkit::MQueue.warn?
            ...
            # :method:warn?
            # :call-seq:
            #   Runkit::MQueue.warn? => true or false
            #   Runkit::MQueue.warn = new_value
            #
            # Controls whether runkit.rb should issue a warning if auto_use? is
            # true but some connection cannot use the message queues
            #
            # See the documentation of Runkit.auto_use? for the constraints on MQ
            # usage.
            ...
            # :method:auto_sizes?
            # :call-seq:
            #   Runkit::MQueue.auto_sizes? => true or false
            #   Runkit::MQueue.auto_sizes = new_value
            #
            # Controls whether runkit.rb should compute the required data size
            ...
            #
            # See the documentation of Runkit.auto? for the constraints on MQ
            ...
            # :call-seq:
            #   Runkit::MQueue.validate_sizes? => true or false
            #   Runkit::MQueue.validate_sizes = new_value
            #
            # Controls whether runkit.rb should validate the data_size field in
            ...
            # :call-seq:
            #   Runkit::MQueue.auto_fallback_to_corba? => true or false
            #   Runkit::MQueue.auto_fallback_to_corba = new_value
            #
            ...
            # instead.
            #
            # While runkit.rb tries very hard to not create MQ connections if it
        ...
        def self.valid_sizes?(buffer_size, data_size)
            if !msg_max || !msgsize_max
                Runkit.warn "the system-level MQ limits msg_max and msgsize_max parameters are unknown on this OS. I am disabling MQ support"
                return false
            end

            if buffer_size > msg_max
                msg = yield if block_given?
                Runkit.warn "#{msg}: required buffer size #{buffer_size} is greater than the maximum that the system allows (#{msg_max}). On linux systems, you can use /proc/sys/fs/mqueue/msg_max to change this limit"
                return false
            end

            if data_size > msgsize_max
                msg = yield if block_given?
                Runkit.warn "#{msg}: required sample size #{data_size} is greater than the maximum that the system allows (#{msgsize_max}). On linux systems, you can use /proc/sys/fs/mqueue/msgsize_max to change this limit"
                return false
            end
            true
        end
