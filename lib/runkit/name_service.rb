...
module Runkit
    ...
    # This should be the default way to acquire an handle to an Runkit Task by
    ...
    #   require 'runkit'
    #   Runkit.initialize
    #   task = Runkit.name_service.get "task_name"
    #
    # @example changing the default underlying CORBA name service
    #   Runkit::CORBA.name_service.ip = "host_name"
    #   Runkit.initialize
    #   task = Runkit.name_service.get 'task_name'
    #
    # @example adding a second CORBA name service
    #   Runkit.name_service << Runkit::CORBA::NameService.new("192.168.101.12")
    #   Runkit.initialize
    #   task = Runkit.name_service.get 'task_name'
    #
    # @example adding a second CORBA name service having a namespace
    #   Runkit.name_service << Runkit::CORBA::NameService.new("192.168.101.12",:namespace => "robot")
    #   Runkit.initialize
    #   task = Runkit.name_service.get 'robot/task_name'
    #
    # @example adding an Avahi name service
    #   Runkit.name_service << Runkit::Avahi::NameService.new("_robot._tcp")
    #   Runkit.initialize
    #   task = Runkit.name_service.get 'task_name'
    #
    # @return [Runkit::NameService] The name service
    ...
    def self.get(name, **options)
        Runkit.name_service.get(name, **options)
    end
    ...
    class NameServiceBase
        ...
        #
        # @return [Runkit::TaskContext,Runkit::Log::TaskContext]
        # @raise [Runkit::NotFound] if no {TaskContext} can be found
        #
        # @see Runkit::NameService
        ...
        # @param [String] name the name of the TaskContext
        # @return [String]
        # @raise [Runkit::NotFound] if the TaskContext cannot be found
        def ior(name)
            raise NotImplementedError
        end
        ...
        def get_provides(type) # :nodoc:
            ...
            if results.empty?
                raise Runkit::NotFound, "no task implements #{type}"
            elsif results.size > 1
                candidates = results.map(&:name).join(", ")
                raise Runkit::NotFound, "more than one task implements #{type}: #{candidates}"
            end
        ...
        def find_one_running(*names)
            ...
            raise Runkit::NotFound, "cannot find any tasks matching #{names.join(', ')}" if candidates.empty?

            running_candidates = candidates.find_all(&:running?)
            if running_candidates.empty?
                raise Runkit::NotFound, "none of #{names.join(', ')} are running"
            elsif running_candidates.size > 1
                raise Runkit::NotFound, "multiple candidates are running: #{running_candidates.map(&:name)}"
            else
    ...
    # there is one global instance accessible via {Runkit.name_service} which
    # has by default one {Runkit::CORBA::NameService} instance as underlying
    # name service.
    #
    # @author Alexander Duda
    #
    # @see Runkit::CORBA.name_service
    # @see Runkit::CORBA::NameService
    # @see Runkit::Avahi::NameService
    #
    class NameService < NameServiceBase
        ...
        # @raise Runkit::NotFound if no name service was added
        attr_reader :name_services
        ...
        def ior(name)
            ...
            name_services.each do |service|
                ...
                begin
                    return service.ior(name) if service.same_namespace?(name)
                rescue Runkit::NotFound
                end
            end
            raise Runkit::NotFound, error_message(name)
        ...
        def names
            names = name_services.collect do |service|
                service.names
            rescue Runkit::CORBAError, Runkit::CORBA::ComError
                []
            end
            names.flatten.uniq
        end
    ...
    module Local
        # Name service which is used by {Runkit::Log::Replay} to register {Runkit::Log::TaskContext} on the
        # global name service {Runkit.name_service}
        # @author Alexander Duda
        class NameService < NameServiceBase
            include Namespace
            ...
            def to_async(options = {})
                Runkit::Async::Local::NameService.new(tasks: registered_tasks)
            end
            ...
            def get(name, options = {})
                ...
                task = @registered_tasks.find do |task|
                    true if [task.name, task.basename].include?(name)
                end
                return task if task
    ...
    module CORBA
        class << self
            ...
            # all Runkit Tasks started by the ruby instance and is by default
            # added to the global Runkit::NameService instance
            # {Runkit.name_service}
            #
            # @return [Runkit::CORBA::NameService] The global CORBA name service
            def name_service
                @name_service ||= NameService.new
            end

            # Sets the default CORBA name service and replaces the old instance stored
            # in {Runkit#name_service} if there is one.
            #
            # @param [Runkit::CORBA::NameService] service the name service
            def name_service=(service)
                if service.respond_to? :to_str
                    # To support deprecated way of setting the host name
                    CORBA.warn "Runkit::CORBA.name_service = 'host_name' is deprecated."
                    CORBA.warn "Use Runkit::CORBA.name_service.ip = 'host_name' instead."
                    name_service.ip = service
                else
                    # check if the old name service is added to the global Runkit.name_service
                    # and replace it with the new one
                    Runkit.name_service.name_services.each_with_index do |i, val|
                        if val == @name_service
                            Runkit.name_service.name_services[i] = service
                            break
                        end
                    end
            ...
            # Calls cleanup on the global Runkit::CORBA::NameService instance {Runkit::CORBA.name_service}
            #
            # @see Runkit::CORBA::NameService.cleanup
            def cleanup
                name_service.cleanup
            end
        end
        ...
        # handle to registered Runkit Tasks.
        # By default there is one global instance accessible via {Runkit::CORBA.name_service}
        # which is also by default added to {Runkit.name_service}
        #
        # The default {Runkit::CORBA.name_service} is used to register all Runkit Tasks started
        # by the ruby instance no matter if it was removed from {Runkit.name_service} or not.
        #
        # @see Runkit.name_service
        # @author Alexander Duda
        class NameService < NameServiceBase
            include Namespace
            ...
            # @param (see Runkit::Async::CORBA::NameService#initialize)
            # @return [Runkit::Async::CORBA::NameService]
            def to_async(reconnect: true)
                Runkit::Async::CORBA::NameService.new(ip, reconnect: reconnect)
            end
            ...
            def get(name = nil, namespace: nil, process: nil, ior: nil)
                if ior
                    Runkit::TaskContext.new(ior, namespace: namespace, process: process)
                else
                    ...
                    namespace ||= (ns || "")
                    Runkit::TaskContext.new(ior(name), namespace: namespace, process: process)
                end
            rescue ComError => e
                raise Runkit::NotFound, "task context #{name} is registered but cannot be reached."
            end
    ...
    module Avahi
        # Name service to access Runkit Tasks which are publishing their IOR via Avahi
        class NameService < NameServiceBase
            ...
            def ior(name)
                ...
                if services.empty?
                    raise Runkit::NotFound, "Avahi nameservice could not find a task named '#{name}'"
                ...
                raise Runkit::NotFound, "Avahi nameservice could not retrieve an ior for task #{name}" if !ior || ior.empty?
            ...
            def get(name, options = {})
                ...
                Runkit::TaskContext.new(ior(name), options)
            rescue Runkit::CORBA::ComError => e
                raise Runkit::NotFound, "task context #{name} is registered but cannot be reached."
            end
        end
    end
