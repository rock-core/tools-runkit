...
module Runkit
    # @deprecated renamed to Runkit::Scripts.watch
    #--
    ...
    # @example
    #   require 'runkit'
    #   require 'runkit/scripts'
    #   options = OptionParser.new do |opt|
    #       opt.banner = "myscript [options]"
    #       Runkit::Scripts.common_optparse_setup(opt)
    #   end
    #   arguments = options.parse(ARGV)
    #   ...
    #   Runkit::Scripts.run 'my::Task' => 'task' do
    #       task = Runkit.get 'task'
    #
    #       # Replaces Runkit.apply_conf, taking into account the command
    #       # line options
    #       Runkit::Scripts.conf(task)
    #
    #       Runkit::Scripts.watch(task)
    #   end
    #
    module Scripts
        class << self
            ...
            # Options that should be passed to Runkit.run
        ...
        def self.common_optparse_setup(optparse)
            ...
            optparse.on("--host=HOSTNAME", String) do |hostname|
                Runkit::CORBA.name_service = hostname.to_str
            end
            ...
            optparse.on("--conf-dir=DIR", String, "load the configuration files in this directory (not needed when using bundles)") do |conf_source|
                Runkit.conf.load_dir(conf_source)
            end
        ...
        def self.conf(task)
            ...
            if file
                Runkit.apply_conf(task, file, sections, true)
            else
                Runkit.conf.apply(task, sections, true)
            end
        end
        ...
        def self.run(*options, &block)
            deployments, models = Runkit::Process.partition_run_options(*options)
            ...
            if deployments.empty? && models.empty?
                ...
                Runkit.run(deployments.merge(models).merge(run_options), &block)
            end
        end
        ...
        #   the method call, e.g.
        #
        #       Runkit::Scripts.watch(task, sleep: 0.1)
        #
        ...
        # otherwise.
        def self.watch(*objects, &block)
            # TODO: move the code here and truly deprecate Runkit.watch
            Runkit.watch(*objects, &block)
        end
    end
end
