        ...
        module Component #:nodoc:
            ...
            def setup
                ...
                self.class.run_specs.each do |name, task_name, run_spec|
                    start(run_spec)
                    instance_variable_set("@#{name}", Runkit.name_service.get(task_name))
                end
            ...
            #         task = Runkit.name_service.get 'task'
            #         task.device = ""
            #         assert_raises(Runkit::StateTransitionFailed) { task.configure }
            #       end
            #   end
            #
            def start(*args)
                processes.concat(Runkit.run(*args))
                nil
            end
