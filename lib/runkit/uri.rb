...
require "uri"
module URI
    class Runkit < URI::Generic
        class << self
            def from_port(port)
                hash = { type_name: port.runkit_type_name }
                Runkit.new("OROCOS", nil, nil, nil, nil, "/port/#{port.full_name}", nil, to_query(hash), nil)
            end
        ...
        def initialize(scheme, userinfo, host, port, registry, path, opaque, query, fragment, parser = DEFAULT_PARSER, arg_check = false)
            ...
            if klass_match = Regexp.new("/(port)/(.+)").match(path)
                ...
                raise ArgumentError, "#{uri} is not a valid path in an runkit URI"
            end
        end
        ...
        def task_proxy
            ...
            ::Runkit::Async.name_service.proxy(task_name)
        end
        ...
        def port_proxy
            ...
            type = if @hash.key? :type_name
                       name = @hash[:type_name]
                       begin
                           ::Runkit.load_typekit_for name
                           ::Runkit.registry.get name
                       rescue Exception => e
                           Vizkit.warn e
                           nil
                       end
...
URI.scheme_list["OROCOS"] ||= URI::Runkit if defined? URI.scheme_list
