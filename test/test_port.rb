...
module Runkit
    describe Port do
        it "checks equality based on CORBA reference" do
            task1 = new_ruby_task_context "task"
            task1.create_output_port "out", "/double"
            p1 = task1.port "out"

            task2 = TaskContext.new(task1.ior, name: "task")
            p2 = task2.port "out"
            refute_same(p1, p2)
            assert_equal(p1, p2)
        end
        ...
        describe "handle_mq_transport" do
            ...
            before do
                ...
                @port = task.create_output_port "out", "/double"
